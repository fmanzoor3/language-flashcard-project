...
export default function FlashcardManager({ onClose }: FlashcardManagerProps) {
  const { cards, updateCard, deleteCard } = useFlashcardStore();
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 rounded-xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-xl border border-slate-700">
        {/* Header */}
        <div className="p-4 border-b border-slate-700 flex items-center justify-between shrink-0">
          <div>
            <h2 className="text-xl font-bold text-slate-100">Manage Flashcards</h2>
          ...
          {filteredCards.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-slate-400">
                {searchQuery ? 'No cards match your search' : 'No flashcards yet'}
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-medium text-white truncate">{card.front}</span>
                        <span className="text-slate-500">→</span>
                        <span className="text-emerald-400 truncate">{card.back}</span>
                      </div>
                      <div className="flex items-center gap-3 text-xs">
                        ...
                        {card.notes && (
                          <span className="text-slate-500 truncate max-w-[150px]" title={card.notes}>
                            📝 {card.notes}
                          </span>
                        )}
                      </div>
